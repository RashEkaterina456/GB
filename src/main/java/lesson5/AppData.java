...
public class AppData {
    ...
    public AppData(String[] newHeader, int[][] newData) {
        if (validateValuesNumber(newData, newHeader.length)) {
            ...
            throw new IllegalArgumentException("Несоответствие между заголовком и длиной данных!");
        }
    }
    ...
    private int[] stringIntoIntArray(String strToConvert, String cvsSplitBy) {
        ...
        for (int i = 0; i < strArray.length; i++) {
            try {
                ...
                System.out.println("Неправильный формат. Строка '" + intArray[i] + "' не может быть преобразован в int!");
            }
        }
        return intArray;
    }
